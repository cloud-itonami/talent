...
> `kotoba/schema.edn`; logic in `src/` and tests in `test/`. The four PII-Tier-3 rules are now
> STRUCTURAL: self-sovereign write (caller = subject), Signal-E2E PII (plaintext refused),
> k-anonymity cohort stats, GDPR Art 17 hard delete. Legacy `wire/actor-manifest.jsonld` (RisingWave)
